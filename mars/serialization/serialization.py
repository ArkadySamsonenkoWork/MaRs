...
class ExperimentalParameters:
    ...
    :param position_shift: Optional tensor that defines a constant offset to be
                               added to the reconstructed grid positions when converting
                               to absolute or physical coordinates. The shift is in the
                               same units as `min_pos` and `max_pos`. If provided, the
                               effective positions are computed as
                               `grid[i] + position_shift` (elementwise). The shift is
                               **not** applied during grid reconstruction (i.e., the
                               grid generated by `reconstruct_grid()` remains within
                               `[min_pos, max_pos]`); it is intended for later
                               coordinate transformations or for storing metadata.
    :type position_shift: tp.Optional[torch.Tensor]
    """
    ...
    position_shift: tp.Optional[torch.Tensor] = field(default=None)
    ...
    def to_dict(self) -> tp.Tuple[tp.Dict[str, tp.Any], tp.Dict[str, torch.Tensor]]:
        ...
        if self.position_shift is not None:
            tensors[f"position_shift"] = self.position_shift
    ...
    def from_dict(cls, meta: tp.Dict[str, tp.Any], tensors: tp.Dict[str, torch.Tensor]) -> ExperimentalParameters:
        ...
        return cls(
            ...
            position_shift=tensors.get("position_shift"),
        )
...
class CWSpectralData:
    ...
    def from_dict(cls, meta: tp.Dict[str, tp.Any], tensors: tp.Dict[str, torch.Tensor]) -> CWSpectralData:
        ...
        experimental_parameters = ExperimentalParameters(
            ...
            num_points=meta["num_points"],
            resonance_parameter=tensors.get("resonance_parameter"),
            position_shift=meta.get("position_shift"),
        )
